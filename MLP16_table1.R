...
nn.1  <- nnet(as.factor(V535)~., data = train_subset,size=16,decay=1e-5,maxit=100,MaxNWts=150000)
predictions5 <- predict(nn.1, test_subset, type = "class")
comparison <- data.frame(actual = as.factor(test_subset$V535), predicted = predictions5)
table(comparison)
...
x_in_test = test[,2:21,200:230,400:430]
y_in_test = test[,535]
x_in_train = train[,2:21,200:230,400:430]
y_in_train = train[,535]
train_subset=train[c(2:21,200:230,400:430,535)]
test_subset = test[c(2:21,200:230,400:430,535)]

nn.1  <- nnet(as.factor(V535)~., data = train_subset,size=16,decay=1e-3,maxit=150,MaxNWts=150000)
predictions5 <- predict(nn.1, test_subset, type = "class")
comparison <- data.frame(actual = as.factor(test_subset$V535), predicted = predictions5)
table(comparison)
...
nn.1  <- nnet(as.factor(V535)~., data = train_subset,size=16,decay=1e-3,maxit=100,MaxNWts=150000)
predictions5 <- predict(nn.1, test_subset, type = "class")
comparison <- data.frame(actual = as.factor(test_subset$V535), predicted = predictions5)
table(comparison)
...
x_in_test = test[,2:11,400:409]
y_in_test = test[,535]
x_in_train = train[,2:11,400:409]
y_in_train = train[,535]
train_subset=train[c(2:11,400:409,535)]
test_subset = test[c(2:11,400:409,535)]

nn.1  <- nnet(as.factor(V535)~., data = train_subset,size=16,decay=1e-1,maxit=100,MaxNWts=150000)
predictions5 <- predict(nn.1, test_subset, type = "class")
comparison <- data.frame(actual = as.factor(test_subset$V535), predicted = predictions5)
table(comparison)
...
x_in_test = test[,200:250,400:430]
y_in_test = test[,535]
x_in_train = train[,200:250,400:430]
y_in_train = train[,535]
train_subset=train[c(200:250,400:430,535)]
test_subset = test[c(200:250,400:430,535)]

nn.1  <- nnet(as.factor(V535)~., data = train_subset,size=16,decay=1e-2,maxit=100,MaxNWts=150000)
predictions5 <- predict(nn.1, test_subset, type = "class")
comparison <- data.frame(actual = as.factor(test_subset$V535), predicted = predictions5)
table(comparison)
